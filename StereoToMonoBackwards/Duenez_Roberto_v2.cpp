...
// Opens the file and prints header and wave file characteristics. 
	...
	if((wavfile[0] == '.')&&(wavfile[1]=='w'||wavfile[1]=='W')&&(wavfile[2]=='a'||wavfile[2]=='A')&&(wavfile[3]=='v'||wavfile[3]=='V')){
		...
		if((fp = fopen(path,"rb" )) == NULL) printf("**CANNOT OPEN FILE!\n\n");
			...
			if(result == 2) {
				...
				strcat(newpath,newfilename);						// newpath is now wav11mono.wav
...
int fileread(FILE *B){
	...
	fread(&header1,44,1,B);
		...
		// Beginning of check to see if uncompressed stereo wav file with 16bit sample rate.
...
printf("datasize: %d\n",header1.datasize);
signed short int *memptr;
// Writing to file fwrite()
count = 0;
memptr = (signed short int*) calloc((header1.datasize/2),2);		//is this the right way?
	fwrite(&header1,44,1,NEWFILE);
	while((fread(&left,2,1,OLDFILE) ) != NULL){
		fread(&right,2,1,OLDFILE);
		*(memptr+count) = left;								
		//memptr = memptr + 1;		//why doesn't this work??
		count++;
		if( count%100 == 0 ) printf("count = %d\n",count);	
	}
	for( x = -1; x != count; count--){
		left = *(memptr+count);
